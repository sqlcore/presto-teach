...
在 SQL 类型中通常有不同的范围和精度要求。例如 SQL 中需要的 DECIMAL 类型需要支持非常大的数字和精确的小数，这在 Java 的原生类型中无法直接表示，这里说的无法表示是说无法用直接原生的 Java 类型。还有就是时间类型，在 SQL 中一般会有多种时间类型的定义，如 `DATE`, `TIME`, `TIMESTAMP`，以及 SQL 中的 NULL 处理与 Java 中的 NULL 也有所不同，这些类型在 Java 中没有直接对应的类型，或者它们的行为与 SQL 标准不完全一致，一般面对这些类型都会重新设计封装。
...
这 2 个方法主要实现一个是对内展示类型的识别，另一个则是对外比如写 SQL 时，显示的 Varchar，Boolean 等等。
